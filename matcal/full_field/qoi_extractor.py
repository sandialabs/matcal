    ...
    def calculate(self, working_data, reference_data, fields):
        ...
        for field in fields:
            ...
            for time_index in range(n_time):
                interp_vars_at_time = self._space_interpolator.map(
                    field_data[time_index, :]
                )
...
class FieldTimeInterpolatorExtractor(QoIExtractorBase):
    ...
    def calculate(self, working_data, reference_data, fields):
        working_nodes = _get_node_coordinate_mapping(
            working_data.skeleton, self._old_grid, self._old_grid.subset_name
        )
        ...
        qoi = interpolate_fields_in_time(reference_time, working_time, field_data)
    ...
    def calculate(self, working_data, reference_data, fields):
        ...
        for field in fields:
            ...
            for t_idx in range(n_time):
                interp_spatial[t_idx, :] = self._space_interpolator.map(
                    field_data[t_idx, :]
                )
...
def _default_velocity_function(points, field_data):
    h = np.max(field_data.spatial_coords[:, 1]) - np.min(
        field_data.spatial_coords[:, 1]
    )
...
class ExternalVirtualPowerExtractor(QoIExtractorBase):
    ...
    def __init__(
        self, time_field, load_field, velocity_function=_default_velocity_function
    ):
        self._time_field = time_field
...
class InternalVirtualPowerExtractor(QoIExtractorBase):
    ...
    def _calculate_volume_scaled_velocity_gradient(self, working_data):
        ...
        centroids = np.array(
            [working_data[0]["centroid_x"], working_data[0]["centroid_y"]]
        ).T
    ...
    def calculate(self, working_data, reference_data, fields):
        ...
        for timestep in range(len(working_data[self._time_field])):
            virtual_velocity_gradient = self._calculate_volume_scaled_velocity_gradient(
                working_data
            )
        ...
        data = convert_dictionary_to_data(
            {
                "virtual_power": interp_internal_power,
    ...
    def _interpolate_evaluation_data_to_projection_data(
        self, reference_time, working_time, internal_power
    ):
        return _time_interpolate(
            reference_time, working_time, np.asarray(internal_power)
        )
...
class HWDPolynomialSimulationSurfaceExtractorBASE(QoIExtractorBase):
    ...
    @abstractmethod
    def hwd_init(self, poly_order: int, depth: int, coords: np.ndarray) -> None:
        """"""

    @abstractmethod
    def _get_field_weights(
        self,
        working_data: "FieldData",
        is_exp: bool,
        point_cloud_working: object,
        time_interp_working_data: object,
        field: str,
    ) -> np.ndarray:
        """"""

    def __init__(
        self, inital_points: MeshSkeleton, depth: int, poly_order: int, time_field: str
    ):
        self._time_field = time_field
    ...
    def calculate(
        ...
        time_interped = interpolate_fields_in_time(time_ref, time_work, parsed_fields)

        weights = {}
        for name in field_names:
            weights[name] = self._get_field_weights(
                working_data, is_exp, point_cloud_working, time_interped[name], name
            )
    ...
    def _parse_point_cloud(self, working_data, is_exp):
        ...
        point_cloud_working = self._down_select(
            working_data, point_cloud_working, is_exp
        )
    ...
    def _map_nodes_if_needed(self, working_data, is_exp):
        if not is_exp and self._is_colocated:
            node_map = _get_node_coordinate_mapping(
                working_data.skeleton,
...
class HWDPolynomialSimulationSurfaceExtractor(
    HWDPolynomialSimulationSurfaceExtractorBASE
):
    ...
    def calculate(
        self,
        working_data: FieldData,
        referecne_data: FieldData,
        field_names: list,
        is_exp: bool = False,
    ) -> Data:
        return super().calculate(working_data, referecne_data, field_names, is_exp)

    def _get_field_weights(
        self, working_data, is_exp, point_cloud_working, time_interp_working_data, field
    ):
        if point_cloud_working is None:
            point_cloud_working = self._parse_point_cloud(working_data, is_exp)
            field_weights = self._hwd.map_data(
                time_interp_working_data.T, point_cloud_working, self._R_ref
            ).T.flatten()
        elif point_cloud_working is not None:
            field_weights = self._hwd.map_data(
                time_interp_working_data.T, R_ref=self._R_ref
            ).T.flatten()
        else:
...
class HWDColocatingExperimentSurfaceExtractor(QoIExtractorBase):
    ...
    def calculate(self, working_data, reference_data, fields):
        mapped_field_data = meshless_remapping(
            working_data,
            fields,
            self._sim_coords,
        ...
        mapped_field_data.set_spatial_coords(
            np.hstack((self._sim_coords, np.zeros((len(self._sim_coords[:, 0]), 1))))
        )
        # self._sim_extractor.hwd_complete(self._sim_coords, mapped_field_data, fields)
        qois = self._sim_extractor.calculate(
            mapped_field_data, mapped_field_data, fields, is_exp=True
        )
...
class FieldInterpolatorExtractor(QoIExtractorBase):
    ...
    def __init__(self, mesh_skeleton, cloud_points, time_field):
        ...
        self._space_interpolator = _TwoDimensionalFieldInterpolator(
            grid_geometry, cloud_points
        )
        self._old_grid = mesh_skeleton
        self._time_field = time_field
        self._node_map = None
