...
from matcal.full_field.objective import (
    InterpolatedFullFieldObjective,
    MechanicalVFMObjective,
    PolynomialHWDObjective,
)
from matcal.full_field.qoi_extractor import (
    ExternalVirtualPowerExtractor,
...
class TestInterpolatedFullFieldObjective(MatcalUnitTest):
    ...
    def test_return_zeros_from_constants(self):
        ...
        sim_times = [0, 0.25, 0.75, 1.5, 3.0]
        sim_data_collection = self._make_spatial_data(
            mesh_loc, fun, sim_times, 0, **mesh_sets
        )
        obj_set = ObjectiveSet(
            ObjectiveCollection("t", iffo),
            exp_data_collection,
            exp_data_collection.states,
            ReturnPassedDataConditioner,
        )

        results_obj, results_qoi = obj_set.calculate_objective_set_results(
            sim_data_collection
        )
    ...
    def _make_spatial_data(self, locations, fun, times, offset, **surfaces):
        raw_data = {
            "time": times,
            "displacement_x": fun(locations["x"], locations["y"], times, offset),
            "displacement_y": fun(locations["x"], locations["y"], times, offset),
        }
        raw_data.update(locations)
        raw_data["surfaces"] = surfaces
        data = convert_dictionary_to_field_data(
            raw_data, ["x", "y"], node_set_name="surfaces"
        )
        data.set_name("first")
    ...
    def _create_square_objective(self):
        ...
        mesh_loc = {
            "x": mesh_skeleton.spatial_coords[:, 0],
            "y": mesh_skeleton.spatial_coords[:, 1],
        }
        fem_surface = "DIC"
        n_points = 40
        dic_loc = {
    ...
    def test_return_unit(self):
        ...
        sim_times = [0, 0.25, 0.75, 1.5, 3.0]
        sim_data_collection = self._make_spatial_data(
            mesh_loc, fun, sim_times, 1, **mesh_sets
        )
        obj_set = ObjectiveSet(
            ObjectiveCollection("t", iffo),
            exp_data_collection,
    ...
    def test_return_linear_two_data_sets(self):
        ...
        obj_set = ObjectiveSet(
            ObjectiveCollection("t", iffo),
            exp_data_collection1,
            exp_data_collection1.states,
            ReturnPassedDataConditioner,
        )
    ...
    def _make_dcs_and_obj(
        ...
        exp_x,
        exp_y,
        init_data_specific=True,
    ):
        exp_data_0 = {
            "x": np.array(exp_x),
            "y": np.array(exp_y),
            "time": np.array(exp_time),
        }
        exp_data_1 = {}
        for key, value in exp_data_0.items():
            exp_data_1[key] = np.multiply(
                value, np.random.uniform(0.9, 1.1, value.shape)
            )
        exp_data_0["T"] = field_func(
            exp_data_0["x"], exp_data_0["y"], exp_data_0["time"]
        )
        exp_data_1["T"] = field_func(
            exp_data_1["x"], exp_data_1["y"], exp_data_1["time"]
        )
        exp_dc = DataCollection(
            "Exp",
            convert_dictionary_to_field_data(exp_data_0, ["x", "y"]),
            convert_dictionary_to_field_data(exp_data_1, ["x", "y"]),
        )
        ...
        sim_data = {
            "x": mesh_geo.spatial_coords[:, 0],
            "y": mesh_geo.spatial_coords[:, 1],
            "surf": mesh_geo.surfaces,
            "time": np.array(sim_time),
        }
        sim_data["T"] = field_func(sim_data["x"], sim_data["y"], sim_data["time"])
        sim_data = convert_dictionary_to_field_data(
            sim_data, ["x", "y"], node_set_name="surf"
        )
        ...
        for state in exp_dc.states:
            for ref_data in exp_dc[state]:
                for work_data in sim_dc[state]:
                    qoi = iffo.simulation_qoi_extractor.calculate(
                        work_data, ref_data, "T"
                    )
                    sim_qoi.add(qoi)
        ...
        for state in exp_dc.states:
            for ref_data in sim_dc[state]:
                for work_data in exp_dc[state]:
                    qoi = iffo.experiment_qoi_extractor.calculate(
                        work_data, ref_data, "T"
                    )
                    exp_qoi.add(qoi)
...
class TestPolynomialHWDObjective(MatcalUnitTest):
    ...
    def test_get_zero_for_same_data_colocated(self):
        ...
        dep_var = "temp"
        raw_data1 = {
            "x": np.array([-1.1, -0.1, -0.9, 1.1, 1.0, 0.9]),
            "y": np.array([1, 1.1, 1.2, 1, 1.1, 1.2]),
            time_var: np.array([1, 2, 3]),
        }
        ...
        hwdo = PolynomialHWDObjective(
            data1, dep_var, max_depth=max_depth, polynomial_order=max_poly
        )
        hwdo.data_specific_initialization(dc_all)
        results_obj, results_qoi = hwdo.calculate(
            dc_all, dc_all, dc_all_copy, dc_all_copy
        )
    ...
    def test_get_zero_for_same_data_none_target_coords(self):
        ...
        hwdo = PolynomialHWDObjective(
            None, dep_var, max_depth=max_depth, polynomial_order=max_poly
        )
        hwdo.data_specific_initialization(dc_all)
        results_obj, results_qoi = hwdo.calculate(
            dc_all, dc_all, dc_all_copy, dc_all_copy
    ...
    def test_get_zero_for_same_data_different_time_and_location(self):
        ...
        raw_data1["temp"] = self._linear(
            raw_data1["x"], raw_data1["y"], raw_data1["time"]
        )
        data1 = convert_dictionary_to_field_data(raw_data1, ["x", "y"])

        raw_data2 = {
            "x": np.array([-1.2, -1.0, -0.85, 1.05, 1.02, 0.93]),
            "y": np.array([1.02, 1.08, 1.17, 1.01, 1.11, 1.21]),
            "time": np.array([0.5, 1.5, 2.5, 3.5]),
        }
        raw_data2["temp"] = self._linear(
            raw_data2["x"], raw_data2["y"], raw_data2["time"]
        )
    ...
    def test_get_zero_for_same_data_different_time_and_location_two_states(self):
        ...
        raw_data11["temp"] = self._linear(
            raw_data11["x"], raw_data11["y"], raw_data11["time"]
        )
        ...
        raw_data12["temp"] = (
            self._linear(raw_data12["x"], raw_data12["y"], raw_data12["time"]) * 2
        )
        ...
        raw_data21["temp"] = self._linear(
            raw_data21["x"], raw_data21["y"], raw_data21["time"]
        )
        ...
        raw_data22["temp"] = (
            self._linear(raw_data22["x"], raw_data22["y"], raw_data22["time"]) * 2
        )
    ...
    def test_extract_data_from_surface_linear(self):
        ...
        for data_dict in [ref_dict, test_dict]:
            data_dict[dep_var] = linear_fun(
                data_dict["x"], data_dict["y"], data_dict[time_var]
            )
        ref = convert_dictionary_to_field_data(ref_dict, ["x", "y"])
        test = convert_dictionary_to_field_data(
            test_dict, ["x", "y", "z"], node_set_name="node_surf"
        )
    ...
    def test_extract_data_from_surface_linear_colocate(self):
        ...
        sim = convert_dictionary_to_field_data(
            sim_dict, ["x", "y", "z"], node_set_name="node_surf"
        )
        ...
        hwdo = PolynomialHWDObjective(
            sim.skeleton, dep_var, max_depth=max_depth, polynomial_order=max_poly
        )
        hwdo.extract_data_from_mesh_surface(s_name)
        obj_set = ObjectiveSet(ObjectiveCollection("test", hwdo), dc_exp, dc_exp.states)
        results_obj, results_qoi = obj_set.calculate_objective_set_results(dc_sim)
...
class TestMechanicalVFMObjective(MatcalUnitTest):
    ...
    def test_get_results_object_from_calculate(self):
        ...
        obj_set = ObjectiveSet(
            ObjectiveCollection("test", obj), dc_fdata, dc_fdata.states
        )

        results_obj, results_qoi = obj_set.calculate_objective_set_results(dc_zero)
        self.assertIsInstance(results_obj[obj.name], ObjectiveResults)
    ...
    def test_get_zeros_from_calculate_with_zeros(self):
        ...
        self.assert_close_arrays(
            goal, results_obj.get_flattened_weighted_conditioned_residuals(), 1e-7
        )
    ...
    def test_get_exp_array_from_calculate_with_zero_stress(self):
        ...
        for resid_i in range(len(goal)):
            self.assertAlmostEqual(
                goal[resid_i],
                results_obj.residuals["matcal_default_state"][0]["virtual_power"][
                    resid_i
                ],
            )
    ...
    def test_confirm_correct_default_qoi_extractor(self):
        ...
        self.assertIsInstance(
            obj_spy.get_experiment_qoi_extractor(), ExternalVirtualPowerExtractor
        )
        self.assertIsInstance(
            obj_spy.get_simulation_qoi_extractor(), InternalVirtualPowerExtractor
        )
...
class TestDeferredInterpolatedFullFieldObjective(MatcalUnitTest):
    ...
    def test_non_deferred_flag_with_mesh_file(self):
        ...
        input_files = "/".join([self.get_current_files_path(__file__), "input_files"])
