...
from matcal.core.boundary_condition_calculators import get_displacement_function_from_strain_data_collection
...
class UniaxialLoadingMaterialPointModel(_StandardSierraModelWithDeathBase):
    ...
    def _additional_boundary_condition_setup(self, state: State) -> None:
        """Additional boundary condition setup (none needed for uniaxial loading)."""

    def _get_loading_boundary_condition_displacement_function(self, state: State, params_by_precedent: Dict[str, Any]) -> Callable:
    ...
    def _add_true_stress_strain_outputs(self) -> None:
        ...
        true_stress_strain_output.add_compute_global_from_element_field(TRUE_STRAIN_KEY, "log_strain(zz)")
        self._input_file._add_heartbeat_global_variable(TRUE_STRAIN_KEY)

        true_stress_strain_output.add_compute_global_from_element_field(TRUE_STRESS_KEY, "cauchy_stress(zz)")
        self._input_file._add_heartbeat_global_variable(TRUE_STRESS_KEY)

        true_stress_strain_output.add_compute_global_from_element_field("log_strain_xx", "log_strain(xx)")
        self._input_file._add_heartbeat_global_variable("log_strain_xx")

        true_stress_strain_output.add_compute_global_from_element_field("log_strain_yy", "log_strain(yy)")
        self._input_file._add_heartbeat_global_variable("log_strain_yy")

        if self.coupling is not None:
            true_stress_strain_output.add_compute_global_from_element_field(TEMPERATURE_KEY, TEMPERATURE_KEY)
            self._input_file._add_heartbeat_global_variable(TEMPERATURE_KEY)

    def _add_contraction_output(self) -> None:
...
class SimpleShearMaterialPointModel(_StandardSierraModelWithDeathBase):
    ...
    def _add_shear_load_outputs(self) -> None:
        ...
        load_output.add_compute_global_from_nodal_field(DISPLACEMENT_KEY, "displacement(x)")
        self._input_file._add_heartbeat_global_variable(DISPLACEMENT_KEY)
        ...
        load_output.add_compute_global_from_nodal_field(LOAD_KEY, "force_external(x)", "sum")
        self._input_file._add_heartbeat_global_variable(LOAD_KEY)
    ...
    def _add_true_stress_strain_outputs(self) -> None:
        ...
        true_output.add_compute_global_from_element_field(TRUE_STRAIN_KEY, "log_strain(xz)")
        self._input_file._add_heartbeat_global_variable(TRUE_STRAIN_KEY)

        true_output.add_compute_global_from_element_field(TRUE_STRESS_KEY, "cauchy_stress(xz)")
        self._input_file._add_heartbeat_global_variable(TRUE_STRESS_KEY)

        # Normal strain components
        true_output.add_compute_global_from_element_field("log_strain_xx", "log_strain(xx)")
        self._input_file._add_heartbeat_global_variable("log_strain_xx")

        true_output.add_compute_global_from_element_field("log_strain_yy", "log_strain(yy)")
        self._input_file._add_heartbeat_global_variable("log_strain_yy")

        true_output.add_compute_global_from_element_field("log_strain_zz", "log_strain(zz)")
        self._input_file._add_heartbeat_global_variable("log_strain_zz")

        # Normal stress components
        true_output.add_compute_global_from_element_field("cauchy_stress_xx", "cauchy_stress(xx)")
        self._input_file._add_heartbeat_global_variable("cauchy_stress_xx")

        true_output.add_compute_global_from_element_field("cauchy_stress_yy", "cauchy_stress(yy)")
        self._input_file._add_heartbeat_global_variable("cauchy_stress_yy")

        true_output.add_compute_global_from_element_field("cauchy_stress_zz", "cauchy_stress(zz)")
        self._input_file._add_heartbeat_global_variable("cauchy_stress_zz")

        # Add temperature output if thermal coupling is active
        if self.coupling is not None:
            true_output.add_compute_global_from_element_field(TEMPERATURE_KEY, TEMPERATURE_KEY)
            self._input_file._add_heartbeat_global_variable(TEMPERATURE_KEY)
