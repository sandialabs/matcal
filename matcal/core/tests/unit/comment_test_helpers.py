...
def assert_comment_line_present(test_case, text, expected_line, commented=True):
    prefix = "# " if commented else ""
    test_case.assertIn(f"{prefix}{expected_line}", text)


def assert_source_fields_comment(test_case, text, dependent_field, uses_time=True, commented=True):
    if uses_time:
        expected = (
    ...
    assert_comment_line_present(test_case, text, expected, commented=commented)


def assert_source_collection_comment(
    test_case,
    text,
    collection_name="boundary conditions",
    commented=True,
):
...
def assert_data_set_index_comment(test_case, text, index=0, commented=True):
    assert_comment_line_present(
        test_case,
        text,
...
def assert_data_set_name_comment(test_case, text, data_set_name, commented=True):
    assert_comment_line_present(
        test_case,
        text,
...
def assert_selection_reason_comment(test_case, text, field_key, state_name, commented=True):
    prefix = "# " if commented else ""
    test_case.assertIn(
        f'{prefix}Selected this data set because its absolute maximum "{field_key}" value of ',
        text,
    )
    test_case.assertIn(
...
def assert_rate_ramp_comment(test_case, text, field_key, rate_key, commented=True):
    prefix = "# " if commented else ""
    test_case.assertIn(
        f'{prefix}Constructed a 2-point linear ramp from the maximum absolute "{field_key}" '
        f'value using "{rate_key}" = ',
        text,
    )


def assert_default_ramp_comment(test_case, text, field_key, commented=True):
    prefix = "# " if commented else ""
    test_case.assertIn(f"{prefix}Constructed default points (0, 0) and (1, ", text)
    test_case.assertIn(
...
def assert_symmetry_displacement_comment(test_case, text, commented=True):
    assert_comment_line_present(
        test_case,
        text,
...
def assert_symmetry_rotation_comment(test_case, text, commented=True):
    assert_comment_line_present(
        test_case,
        text,
...
def assert_rotation_to_radians_comment(test_case, text, commented=True):
    assert_comment_line_present(
        test_case,
        text,
...
def assert_rotation_rename_comment(
    test_case,
    text,
    rotation_field,
    displacement_field,
