...
from matcal.core.data import (
    convert_dictionary_to_data,
    Data,
    DataCollection,
    scale_data_collection,
    UnequalTimeDimensionSizeError,
    _confirm_first_dimension_length,
    _determine_data_type,
    convert_data_to_dictionary,
    _scale_data,
    RangeDataConditioner,
    MaxAbsDataConditioner,
    ReturnPassedDataConditioner,
    Scaling,
    ScalingCollection,
    AverageAbsDataConditioner,
    DataCollectionStatistics,
    combine_data_sets_in_data_list,
)
...
class CommonDataUnitTests(object):
    ...
    class CommonSetUp(MatcalUnitTest):
        ...
        def setUp(self):
            ...
            self._len = 10
            two_col_array = np.zeros(
                self._len, dtype=[("x", np.double), ("y", np.double)]
            )
    ...
    class CommonTests(CommonSetUp):
        ...
        def test_returns_correct_values(self):
            ...
            self.assert_close_arrays(self._data["y"], np.linspace(0.25, 11, self._len))
        ...
        def test_remove_field_result_has_unique_id_number(self):
            # remove_field goes through NumPy fancy-indexing + copy, which
            # calls __array_finalize__ instead of __new__. Without an explicit
            # fix, the copy inherits the original's _id_number and the two
            # distinct objects share the same identifier.
            original = self._data
            result = original.remove_field("y")
            self.assertIsNot(original, result)
            self.assertNotEqual(
                original._id_number,
                result._id_number,
                "remove_field result shares _id_number with the original",
            )

        def test_confirm_field_names_no_fields(self):
            data = self._data.remove_field("x")
            data = data.remove_field("y")
            self.assertTrue([] == data.field_names)

        def test_remove_last_field_preserves_row_count(self):
            # When the final field is removed the empty-path code creates the
            # result array. The row count of the original must be preserved so
            # that callers relying on .length after the operation are not misled.
            original_length = self._data.length
            result = self._data.remove_field("x")
            result = result.remove_field("y")
            self.assertEqual(
                result.length,
                original_length,
                "remove_field lost row count when removing the last field",
            )
        ...
        def test_data_rename_field(self):
            ...
            self.assertIn(
                "already exists",
                str(ctx.exception),
                "rename_field to existing name should give a clear error message",
            )
        ...
        def test_pass_field_data(self):
            n_pts = 5
            two_col_array = np.zeros(
                self._len, dtype=[("t", np.double), ("u", np.double, (n_pts,))]
            )
        ...
        def test_add_field_duplicate_name_raises_ValueError(self):
            # add_field must raise ValueError when the field name already
            # exists rather than silently overwriting the existing data.
            dict_data = {"x": np.linspace(0, 2, 100), "y": np.linspace(0, 1, 100)}
            my_data = convert_dictionary_to_data(dict_data)
            with self.assertRaises(ValueError):
                my_data.add_field("x", np.linspace(0, 5, 100))

        def test_add_field_preserves_name(self):
            # add_field must preserve the original object's name on the result.
            # Without the fix convert_dictionary_to_data assigns a fresh
            # auto-generated name, silently discarding the original.
            dict_data = {"x": np.linspace(0, 2, 100), "y": np.linspace(0, 1, 100)}
            my_data = convert_dictionary_to_data(dict_data)
            my_data.set_name("my_experiment")
            result = my_data.add_field("z", np.zeros(100))
            self.assertEqual(
                result.name,
                "my_experiment",
                "add_field did not preserve the original name",
            )
...
class DataCollectionTest(MatcalUnitTest):
    ...
    def test_dumps_with_string_dtype_field(self):
        arr = np.zeros(3, dtype=[("x", np.double), ("label", "U10")])
        arr["x"] = [1.0, 2.0, 3.0]
        arr["label"] = ["a", "b", "c"]
        state = State("s1")
        d = Data(arr, state)
        dc = DataCollection("string_dc", d)
        dump = dc.dumps()
        self.assertIn("s1", dump)
        self.assertEqual(len(dump["s1"]), 1)
        self.assertEqual(dump["s1"][0]["x"], [1.0, 2.0, 3.0])
        self.assertEqual(dump["s1"][0]["label"], ["a", "b", "c"])

    def test_dumps_ignore_point_data_skips_multi_column_fields(self):
        n_pts = 5
        arr = np.zeros(3, dtype=[("t", np.double), ("u", np.double, (n_pts,))])
        arr["t"] = [1.0, 2.0, 3.0]
        arr["u"] = np.ones((3, n_pts))
        state = State("s1")
        d = Data(arr, state)
        dc = DataCollection("point_dc", d)
        dump = dc.dumps(ignore_point_data=True)
        self.assertIn("t", dump["s1"][0])
        self.assertNotIn(
            "u",
            dump["s1"][0],
            "Multi-column field should be skipped with ignore_point_data=True",
        )
    ...
    def test_data_collection_get_plot_numbered_label(self):
        dc = self.collection_type("ex", self.d_0, self.d_2)
        state = self.example_state1
        # (#) replaced with "{state.name} {1-based index}"
        label = dc._get_plot_label("data (#)", "data_name", 0, 0, state)
        self.assertEqual(label, f"data {state.name} 1")

        label2 = dc._get_plot_label("data (#)", "data_name", 1, 1, state)
        self.assertEqual(label2, f"data {state.name} 2")

        default_label = dc._get_plot_label("data (#) yay", "data_name", 0, 0, state)
        self.assertEqual(default_label, f"data {state.name} 1 yay")

        # state=None path — label uses only the 1-based index
        label_no_state = dc._get_plot_label("data (#)", "data_name", 0, 0, state=None)
        self.assertEqual(label_no_state, "data 1")
    ...
    def test_plain_label_suppressed_after_first_across_multiple_states(self):
        # label_count is passed by value to _plot_state_data_list so
        # increments inside it are never seen by the caller. When a plain
        # string label is used across a two-state collection the label is
        # emitted once per state (unsuppressed) instead of once total,
        # producing duplicate legend entries.
        # A two-state collection with one dataset each and labels="run" should
        # produce exactly one unsuppressed label ("run") and one suppressed
        # label ("_run"). We capture calls to plt.plot to inspect the labels.
        dc = self.collection_type("ex", self.d_0, self.d_2)
        labels_seen = []
        real_plot = plt.plot
        ...
        plt.close("all")
        with self.subTest("label_count propagates across states"):
            dc.plot("U", "F", labels="run", show=False, plot_function=mock_plot)
        unsuppressed = [l for l in labels_seen if not l.startswith("_")]
        self.assertEqual(
            len(unsuppressed), 1, f"Expected 1 unsuppressed label, got {labels_seen}"
        )

    def test_plot_state_data_list_empty_list_does_not_crash(self):
        # _plot_state_data_list must not crash when called with an empty list.
        import matplotlib.pyplot as _plt

        dc = self.collection_type("ex", self.d_0)
        _plt.close("all")
        # Call _plot_state_data_list directly with an empty list.
        _plt.figure()
        dc._plot_state_data_list([], _plt, _plt.plot, "F", "U", None, None, 0, {})

    def test_plot_raises_import_error_when_matplotlib_missing(self):
        from unittest.mock import patch
        import matcal.core.data as data_mod

        dc = self.collection_type("ex", self.d_0)
        with patch.object(data_mod, "plt", None):
            with self.assertRaises(ImportError):
                dc.plot("U", "F")

    def test_describe_states_with_params(self):
        dc = self.collection_type("ex", self.d_0)
        result = dc._format_states_for_warning()
        self.assertIn("example", result)
        self.assertIn("val=", result)

    def test_describe_states_no_params(self):
        d = convert_dictionary_to_data({"x": [1.0, 2.0]})
        dc = DataCollection("no_params", d)
        result = dc._format_states_for_warning()
    ...
    def test_state_field_names(self):
        ...
        self.assertEqual(len(state1_field_names_state_key), len(state1_goal))

        for entry in state2_goal:
            self.assertTrue(entry in state2_field_names_state_key)
        self.assertEqual(len(state2_field_names_state_key), len(state2_goal))

    def test_state_common_field_names(self):
        dc = self.collection_type("Mixed", self.d_0, self.d_1, self.d_2, self.d_t)
    ...
    def test_select_data_by_state_values(self):
        ...
        # All states match — result should contain the same states as original.
        self.assertEqual(dc.state_names, sub_dc.state_names)
        ...
        with self.assertLogs("matcal.core.data", level=logging.WARNING) as log:
            sub_dc = dc.get_data_by_state_values(val3=1, val1=20)
        self.assertTrue(sub_dc.state_names == [])
        self.assertTrue(
            any("no states" in m.lower() for m in log.output),
            "Expected a warning when no states match",
        )

    def test_get_data_by_state_values_all_match_returns_independent_copy(self):
        # When all states match, get_data_by_state_values returns self
        # directly. Mutating the result silently modifies the original.
        dc = self.collection_type("Mixed", self.d_0, self.d_1, self.d_2)
        original_state_count = len(dc.state_names)
        sub_dc = dc.get_data_by_state_values(val=1)
        # Confirm all states matched (the aliasing shortcut was taken)
        self.assertEqual(len(sub_dc.state_names), original_state_count)
        # Mutate sub_dc — should not affect dc
        sub_dc.set_name("modified")
        self.assertNotEqual(
            dc.name,
            "modified",
            "get_data_by_state_values returned self; mutation affected the original",
        )
    ...
    def test_has_report_statistics_reports_interp_locations_across_min_and_max_all_data(
        self,
    ):
        p_means = {"a": 2, "b": 10}
        p_sd = {"a": 1, "b": 2}
    ...
    def test_interpolation_error_chains_original_exception(self):
        # _generate_interpolated_data_per_state_by_field raises RuntimeError
        # via _raise_stats_error without chaining the original exception.
        # The original traceback and cause are lost, making failures hard
        # to debug. The RuntimeError must be raised with `from e` so that
        # __cause__ is set.
        dc = DataCollection(
            "test",
            *[
                convert_dictionary_to_data({"x": np.linspace(0, 1, 5), "y": np.ones(5)})
                for _ in range(3)
            ],
        )
        state = list(dc.keys())[0]

        class _BrokenInterpolator:
            def __init__(self, x, y):
                raise ValueError("deliberate interpolation failure")

        stats = DataCollectionStatistics()
        stats.set_interpolation_tool(_BrokenInterpolator)
        with self.assertRaises(RuntimeError) as ctx:
            stats.generate_state_statistics("x", dc, state)
        self.assertIsNotNone(
            ctx.exception.__cause__,
            "RuntimeError must chain the original exception via 'raise ... from e'",
        )
...
class TestDictionaryToData(MatcalUnitTest):
    ...
    def test_recreate_simple_data(self):
        for fname in self.simple_data.field_names:
            self.assertTrue(
                np.allclose(self.simple_data[fname], self.simple_data_dict[fname])
            )
    ...
    def test_read_in_dict(self):
        ...
        self.assertEqual(data.field_names, ["A", "Beta"])
...
class TestDataToDictionary(MatcalUnitTest):
    ...
    def test_combine_data_sets_does_not_alias_source_arrays(self):
        d1 = convert_dictionary_to_data({"x": [1.0, 2.0], "y": [3.0, 4.0]})
        combined = combine_data_sets_in_data_list([d1])
        combined["x"][0] = 999.0
        self.assertAlmostEqual(
            d1["x"][0],
            1.0,
            msg="Mutating combined dict should not affect the source Data",
        )
...
def get_conditioner_test_data():
    ...
    simple_data_dict2 = {
        "time": np.linspace(0.4, 0.8, 5),
        "value": np.linspace(2, 4, 5),
    }
...
class ConditionerBaseTests:
    class CommonTests(MatcalUnitTest):
        ...
        def setUp(self):
            ...
            # Note: indices are intentionally cross-mapped to historical fixture naming.
            # data_tup = (simple_data, simple_data2, simple_data3, complex_data)
        ...
        def test_returned_conditioned_data(self):
            ...
            scale_time, offset_time = self.get_scale_and_offset(
                self._simple_data["time"]
            )
            scale_value, offset_value = self.get_scale_and_offset(
                self._simple_data["value"]
            )
            ...
            self.assertTrue(np.allclose(goal_value, cond_data["value"]))
        ...
        def test_return_complex_data_with_a_zero_range(self):
            ...
            scale_zero, offset_zero = self.get_scale_and_offset(
                self._complex_data["zeros"]
            )
            ...
            goal_zero_range = (
                self._complex_data["zero_range"] - offset_zero_range
            ) / scale_zero_range

            self.assertTrue(np.allclose(goal_time, cond_data["time"]))
            self.assertTrue(np.allclose(goal_value, cond_data["value"]))
            self.assertTrue(np.allclose(goal_zero, cond_data["zeros"]))
            self.assertTrue(np.allclose(goal_zero_range, cond_data["zero_range"]))

        def test_return_condition_dat_with_multi_data_init(self):
            dc = self.conditioner_class()
            dc.initialize_data_conditioning_values(
                [self._complex_data, self._simple_data]
            )

            cond_data = dc.apply_to_data(self._complex_data)
            scale_time, offset_time = self.get_scale_and_offset(
                self._simple_data["time"], self._complex_data["time"]
            )
            scale_value, offset_value = self.get_scale_and_offset(
                self._simple_data["value"], self._complex_data["value"]
            )
            scale_zero, offset_zero = self.get_scale_and_offset(
                self._complex_data["zeros"]
            )
        ...
        def test_apply_first_conditioning_to_new_data_and_noise(self):
            ...
            self.assertTrue(
                np.allclose(
                    (data2["value"] - offset_value) / scale_value, cond_data["value"]
                )
            )
            self.assertTrue(
                np.allclose(
                    data2["value_noise"] / scale_value, cond_data["value_noise"]
                )
            )

        def test_apply_first_conditioning_to_new_data_and_noise_missing_noise_conditioning_field(
            self,
        ):
            data1, data2 = get_conditioner_test_data_with_noise()
            dc = self.conditioner_class()
            ...
            data_dict = {
                ...
                "unique": np.random.uniform(0, 1, 10),
                "unique_noise": np.ones(10),
            }
            data_with_noise_field_not_in_conditioner = convert_dictionary_to_data(
                data_dict
            )
            ...
            for field in cond_data.field_names:
                ...
                self.assertTrue(
                    np.allclose(
                        cond_data[field],
                        (data_with_noise_field_not_in_conditioner[field] - offset)
                        / scale,
                    )
                )
        ...
        def test_noise_field_without_underscore_prefix_raises_value_error(self):
            # A field literally named "noise" passes _is_noise_field but
            # _get_non_noise_field_name returns "" — there is no corresponding
            # data field. The conditioner should raise ValueError.
            data_dict = {"time": np.linspace(0, 1, 11), "noise": np.ones(11) * 0.5}
            data = convert_dictionary_to_data(data_dict)
            dc = self.conditioner_class()
            dc.initialize_data_conditioning_values([data])
            with self.assertRaises(ValueError):
                dc.apply_to_data(data)

        def test_negative_scale_from_subclass_is_rejected(self):
            # _save_data_conditioning_values_for_field must reject a negative
            # scale returned by get_scale_for_data_field. A negative scale
            # would produce sign-flipped conditioning with no warning.
            # This test patches get_scale_for_data_field to return -1.0 and
            # verifies the conditioner raises ValueError rather than storing
            # the negative scale.
            from unittest.mock import patch

            dc = self.conditioner_class()
            with patch.object(dc, "get_scale_for_data_field", return_value=-1.0):
                with self.assertRaises(ValueError):
                    dc.initialize_data_conditioning_values([self._simple_data])

        def test_integer_columns_promoted_to_float_after_conditioning(self):
            # _initialize_conditioned_array checks dtype equality against
            # np.dtype('int') which only matches the platform default C int.
            # Explicitly-typed columns like int32, int64, uint8 are not matched
            # and remain as integers, causing silent truncation in conditioning
            # arithmetic. All integer subtypes must be promoted to float.
            int_dtypes = [np.int32, np.int64, np.int16, np.int8, np.uint8]
            for int_dtype in int_dtypes:
                with self.subTest(dtype=int_dtype):
                    arr = np.array(
                        [(1, 2), (3, 4), (5, 6)],
                        dtype=[("time", int_dtype), ("value", int_dtype)],
                    )
                    data = Data(arr)
                    dc = self.conditioner_class()
                    dc.initialize_data_conditioning_values([data])
                    result = dc.apply_to_data(data)
                    self.assertTrue(
                        np.issubdtype(result.dtype["time"], np.floating),
                        f"Expected float dtype for 'time' with input {int_dtype}, "
                        f"got {result.dtype['time']}",
                    )

        def test_bool_columns_promoted_to_float_after_conditioning(self):
            # _initialize_conditioned_array guards with np.issubdtype(..., np.integer)
            # which returns False for np.bool_. A bool_ column is kept as bool_,
            # and conditioning arithmetic silently truncates the result back to bool.
            arr = np.array(
                [(True, 1.0), (False, 2.0), (True, 3.0)],
                dtype=[("flag", np.bool_), ("value", np.float64)],
            )
            data = Data(arr)
            dc = self.conditioner_class()
            dc.initialize_data_conditioning_values([data])
            result = dc.apply_to_data(data)
            self.assertTrue(
                np.issubdtype(result.dtype["flag"], np.floating),
                f"Expected float dtype for bool_ column 'flag', got {result.dtype['flag']}",
            )

        def test_reinitialize_uses_new_data_not_corrupted_state(self):
            # initialize_data_conditioning_values appends to _field_names
            # without clearing it first. After a second call the list contains
            # duplicate field names. This also means _verify_valid_initialization
            # cannot detect an empty second init if the first left names behind.
            # The conditioner state must reflect only the most recent init call.
            dc = self.conditioner_class()
            dc.initialize_data_conditioning_values([self._simple_data])

            # Re-initialize with data that has a different range.
            arr2 = np.zeros(3, dtype=[("time", float), ("value", float)])
            arr2["time"] = [100.0, 200.0, 300.0]
            arr2["value"] = [10.0, 20.0, 30.0]
            data2 = Data(arr2)
            dc.initialize_data_conditioning_values([data2])

            # _field_names must not contain duplicates after re-initialization.
            self.assertEqual(
                len(dc._field_names),
                len(set(dc._field_names)),
                "_field_names contains duplicates after re-initialization",
            )

            # Must be able to apply correctly after re-initialization.
            result = dc.apply_to_data(data2)
...
class ScalingTest(MatcalUnitTest):
    ...
    def setUp(self):
        ...
        super().setUp(__file__)
        y = {
            "dependent variable": np.array([-1, 0, 1, 2]),
    ...
    def test_init(self):
        s = Scaling("load")
        self.assertIsInstance(s, Scaling)
    ...
    def test_no_scale(self):
        ...
        self.assertAlmostEqual(
            np.sum(
                np.abs(
                    np.array(data_new["dependent variable"])
    ...
    def test_scalar_scale(self):
        ...
        self.assertAlmostEqual(
            np.sum(np.abs(np.array(ynew["dependent variable"]) - ygoal)), 0, delta=1e-10
        )
...
class TestScaledDataFunction(MatcalUnitTest):
    ...
    def test_take_in_scaling_collection_and_data(self):
        ...
        result = _scale_data(sc, self._complex_data)
        self.assertIsInstance(result, Data)
    ...
    def test_return_doubled_plus_1_value(self):
        sc = ScalingCollection("model", Scaling("value", 2, 0.5))
        s_data = _scale_data(sc, self._complex_data)
        old_data = self._complex_data
        for key in old_data.field_names:
            if key == "value":
...
class TestScaleDataCollection(MatcalUnitTest):
    ...
    def test_bad_inputs_and_raise_error(self):
        ...
        with self.assertRaises(TypeError):
            new_data_collection = scale_data_collection(
                self._data_collection, "value", "val"
            )
        with self.assertRaises(TypeError):
            new_data_collection = scale_data_collection(
                self._data_collection, "value", 1, "1"
            )
    ...
    def test_offset_and_scale_result(self):
        new_data_collection = scale_data_collection(
            self._data_collection, "value", 2, -1
        )
        new_data = new_data_collection[self._state][0]
        old_data = self._complex_data
        for field in old_data.field_names:
            if field == "value":
                ...
                mult = 1
            self.assertTrue(
                np.allclose(new_data[field], mult * (old_data[field] + offset))
            )
...
class TestDataCollectionStatistics(MatcalUnitTest):
    ...
    def test_set_number_of_interpolation_points(self):
        data1 = convert_dictionary_to_data(
            {"indep": np.linspace(0, 1, 100), "dep": np.linspace(0, 10, 100)}
        )
        data2 = convert_dictionary_to_data(
            {"indep": np.linspace(0, 1, 300), "dep": np.linspace(0, 10, 300)}
        )
    ...
    def test_set_number_of_interpolation_points_none_resets_to_default(self):
        data1 = convert_dictionary_to_data(
            {"indep": np.linspace(0, 1, 100), "dep": np.linspace(0, 10, 100)}
        )
        data2 = convert_dictionary_to_data(
        ...
        dc = DataCollection("test", data1, data2)
        stats = DataCollectionStatistics()
        default_pts = stats._get_number_of_field_points("indep", dc[SolitaryState()])
        stats.set_number_of_interpolation_points(5)
        self.assertEqual(
            stats._get_number_of_field_points("indep", dc[SolitaryState()]), 5
        )
        stats.set_number_of_interpolation_points(None)
        reset_pts = stats._get_number_of_field_points("indep", dc[SolitaryState()])
        self.assertEqual(reset_pts, default_pts)
    ...
    def test_generate_statistics_default(self):
        ...
        dc = DataCollection("test", data1, data2)
        stats = DataCollectionStatistics()
        stats_results = stats.generate_state_statistics("indep", dc, SolitaryState())
        self.assert_close_arrays(np.linspace(0, 10, 200), stats_results["dep"]["mean"])
        self.assert_close_arrays(np.zeros(200), stats_results["dep"]["std dev"])
        data1 = convert_dictionary_to_data(
            {"indep": np.linspace(0, 1, 100), "dep": np.linspace(0, 20, 100)}
        )
        data2 = convert_dictionary_to_data(
            {"indep": np.linspace(0, 1, 300), "dep": np.linspace(0, 40, 300)}
        )
        dc = DataCollection("test", data1, data2)
        stats_results = stats.generate_state_statistics("indep", dc, SolitaryState())
        self.assert_close_arrays(np.linspace(0, 30, 200), stats_results["dep"]["mean"])
        self.assert_close_arrays(
            np.linspace(0, 1, 200) * np.std([20, 40]), stats_results["dep"]["std dev"]
        )

    def test_generate_statistics_errors_and_percentiles(self):
        stats = DataCollectionStatistics()
        ...
        dc = DataCollection("test", data1, data2)
        with self.assertRaises(KeyError):
            stats_results = stats.generate_state_statistics(
                "bad_field", dc, SolitaryState()
            )
        bad_state = State("bad state")
        with self.assertRaises(KeyError):
            stats_results = stats.generate_state_statistics("indep", dc, bad_state)
        ...
        self.assert_close_arrays(
            np.linspace(0, 20, 200), stats_results["dep"]["percentile_0"]
        )
        self.assert_close_arrays(
            np.linspace(0, 40, 200), stats_results["dep"]["percentile_100"]
        )
    ...
    def test_set_sort_ascending(self):
        ...
        data1 = convert_dictionary_to_data(
            {"indep": [1.0, 0.5, 2.0, 3.0, 1.5], "dep": [0, 1, 2, 3, 4]}
        )
        data2 = convert_dictionary_to_data(
            {"indep": np.linspace(0, 1, 300), "dep": np.linspace(0, 40, 300)}
        )
