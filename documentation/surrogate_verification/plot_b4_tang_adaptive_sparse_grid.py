r"""
Tang Verification: Adaptive Sparse-Grid Surrogate
=================================================

This example verifies MatCal's adaptive sparse-grid surrogate workflow on a
...
2. Diagnostic plots at 250, 500, and 750 samples showing the true function and
...
WORKING_DIRECTORY = os.path.abspath("paper_tang_adaptive_sparse_grid")
...
if not HAS_PYAPPROX:
    raise ImportError(
        ...
        "sparse-grid Tang verification example."
    )
...
print("Generating fixed Tang validation set.")
...
# ----------------------------------------
#
# Sparse-grid adaptive refinement can add samples in batches. The discontinuity
# in the Tang-style benchmark can require substantial refinement, so we set the
# adaptive stopping threshold to 750 samples. The final number of evaluations
# can exceed this value because sparse-grid refinement adds samples in batches.
...
# ------------------------------------------------------
#
# A piecewise quadratic basis is used for consistency with the smooth-function
# sparse-grid verification example. The discontinuity may still limit the
...
# sparse-grid surrogates near those sample counts. Sparse-grid adaptive batches
# may not land exactly on these counts, so the plotting helper selects the
# nearest retained surrogate.
...
study.set_surrogate_save_filename(
    os.path.join(
        WORKING_DIRECTORY,
        "tang_adaptive_sparse_grid_surrogate.joblib",
    )
)
...
print("Running adaptive sparse-grid Tang verification.")
...
plot_convergence_history(
    ...
    filename="tang_adaptive_sparse_grid_convergence.png",
...
plot_function_and_surrogate_error_at_counts(
    ...
    figure_directory=FIGURE_DIRECTORY,
    filename="tang_adaptive_sparse_grid_error_fields.png",
    method_name="adaptive sparse grid",
    n_grid=150,
)
